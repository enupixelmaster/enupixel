...
void eospixels::clearpixels(uint16_t count, uint16_t nonce) {
  ...
  enumivo_assert(itr != canvases.end(), "no canvas exists");
...
void eospixels::onTransfer(const currency::transfer &transfer) {
  ...
  enumivo_assert(canvasItr != canvases.end(), "game not started");
  auto canvas = *canvasItr;
  enumivo_assert(!canvas.isEnded(), "game ended");

  auto from = transfer.from;
  auto accountItr = accounts.find(from);
  enumivo_assert(accountItr != accounts.end(),
  ...
  enumivo_assert(paintSuccessPercent >= 80, "Too many pixels did not paint.");
...
void eospixels::end() {
  ...
  auto c = *itr;
  enumivo_assert(c.isEnded(), "canvas still has time left");
...
void eospixels::createacct(const account_name account) {
  ...
  enumivo_assert(itr == accounts.end(), "account already exist");
...
void eospixels::init() {
  ...
  enumivo_assert(canvases.begin() == canvases.end(), "already initialized");
...
void eospixels::withdraw(const account_name to) {
  ...
  auto canvasItr = canvases.begin();
  enumivo_assert(canvasItr != canvases.end(), "no canvas exists");

  auto canvas = *canvasItr;
  enumivo_assert(canvas.pixelsDrawn >= WITHDRAW_PIXELS_THRESHOLD,
               "canvas still in game initialization");

  auto acctItr = accounts.find(to);
  enumivo_assert(acctItr != accounts.end(), "unknown account");

  auto guardItr = guards.begin();
  enumivo_assert(guardItr != guards.end(), "no withdraw guard exists");
  ...
  action(permission_level{_self, N(active)}, N(enu.token), N(transfer),
         std::make_tuple(_self, to, quantity,
                         std::string("Withdraw from ENU Pixels")))
      .send();
}

void eospixels::deposit(const account_name user,
                        const uint128_t quantityScaled) {
  enumivo_assert(quantityScaled > 0, "must deposit positive quantity");
...
void eospixels::apply(account_name contract, action_name act) {
  ...
  // needed for ENUMIVO_API macro
  auto &thiscontract = *this;
  switch (act) {
    // first argument is name of CPP class, not contract
    ENUMIVO_API(eospixels, (init)(refresh)(changedur)(end)(createacct)(withdraw)(
