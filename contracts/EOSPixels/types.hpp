...
struct pixel {
  ...
  uint64_t nextPrice() const {
    ...
    enumivo_assert(priceCounter <= 25, "price for pixel is too high");
  ...
  uint8_t nextPriceCounter() const {
    ...
    enumivo_assert(c > 0, "price counter overflow");
...
struct account {
  ...
  ENULIB_SERIALIZE(account, (owner)(balanceScaled)(pixelsDrawn)(maskScaled))
};
...
struct guard {
  ...
  ENULIB_SERIALIZE(guard, (id)(quota))
};
...
struct canvas {
  ...
  ENULIB_SERIALIZE(canvas, (id)(lastPaintedAt)(duration)(lastPainter)(
...
struct pixel_row {
  ...
  ENULIB_SERIALIZE(pixel_row, (row)(pixels))
};
...
uint64_t calculateWithdrawalAndUpdate(const canvas &cnv, account &player,
  ...
  enumivo_assert(withdrawAmount > 0, "Balance too small for withdrawal");

  enumivo_assert(grd.quota >= withdrawAmount,
...
struct st_pixelOrder {
  ...
  void parse(const std::string &memo) {
    ...
    enumivo_assert(y < MAX_COORDINATE_Y_PLUS_ONE, "invalid y");
    enumivo_assert(x < MAX_COORDINATE_X_PLUS_ONE, "invalid x");
  }
...
struct st_transferContext {
  ...
  st_buyPixel_result purchase(const pixel &pixel,
    ...
    enumivo_assert(amountLeft >= nextPrice, "insufficient fund to buy pixel");
  ...
  void updateCanvas(canvas &cv) {
    ...
    enumivo_assert(bonusPerPixelScaled > 0, "bonus is 0");

    cv.maskScaled += bonusPerPixelScaled;
    enumivo_assert(cv.maskScaled >= bonusPerPixelScaled, "canvas mask overflow");
  ...
  void updatePurchaserAccount(account &acct) {
    ...
    enumivo_assert(acct.maskScaled >= maskUpdate, "player mask overflow");
